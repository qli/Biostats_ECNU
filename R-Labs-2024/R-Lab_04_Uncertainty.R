### R Lab 04 - Measuring Uncertainty
### Qin Li 20241015
...
humanGeneLengths = read.csv("R-Labs/chap04e1HumanGeneLengthsLongestTranscript.csv")
head(humanGeneLengths)
str(humanGeneLengths)
hist(humanGeneLengths$size)
...
library(tidyverse)
dt2 = humanGeneLengths %>% filter(size <= 15000)

#dt2 %>% summarise(mean = mean(size),
#                  sd = sd(size)) %>% 
#    mutate(se = ....)
...
sd(geneSample100) 
se = sd(geneSample100) / sqrt(n)# standard error
...
sample_dt = data.frame(results100)
ggplot(data = sample_dt, aes(x = results100)) + 
