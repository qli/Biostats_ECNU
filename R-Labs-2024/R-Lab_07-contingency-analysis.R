...
### Titanic data
#--------------------------------------
titanicData <- read.csv("R-Labs/chap09f01Titanic.csv")
...
#--------------------------------------
# Fisher’s exact test for fish data

# Q: write codes -- fish eaten by bird
fish_data = data.frame(uninfected = c(1,49),
                       lightly_infected = c(10,35),
                       highly_infected = c(37,9))

row.names(fish_data) = c("eaten","not_eaten")

fish_data

chisq.test(fish_data)
chisq.test(fish_data)$expected
